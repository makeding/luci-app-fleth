...
# Contributors: huggy <fleth@huggy.moe>
...
# Ports that must not be used for MAP-E SNAT. Keep this as a space-delimited
...
fleth_map_write_debug_rules() {
	local cfg="$1"
	local k="$2"

	{
		echo "FLETH_RULE_${k}_IPV6ADDR=$(eval "echo \$RULE_${k}_IPV6ADDR")"
		echo "FLETH_RULE_${k}_PORTSETS='$(eval "echo \$RULE_${k}_PORTSETS")'"
	} >> "/tmp/map-$cfg.rules"
}

fleth_map_apply_nft_snat() {
	local cfg="$1"
	local k="$2"
	local portcount=0
	local allports=""
...
proto_map_setup() {
	...
	k=$RULE_BMR
	fleth_map_fix_explicit_psid "$k"
	fleth_map_write_debug_rules "$cfg" "$k"
	...
	json_add_array firewall
	  if [ -z "$(eval "echo \$RULE_${k}_PORTSETS")" ]; then
	    ...
	    fleth_map_apply_nft_snat "$cfg" "$k"
	  fi
	  if [ "$maptype" = "map-t" ]; then
...
proto_map_teardown() {
	...
	case "$maptype" in
		"map-e"|"lw4o6")
			ifdown "${cfg}_"
			nft list tables 2>/dev/null | grep -q "table inet mape" && nft delete table inet mape
			;;
